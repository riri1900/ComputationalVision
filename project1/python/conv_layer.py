...
def conv_layer_forward(input_data, layer, param):
    ...
    conv = im2col_conv_batch(input_n, layer, h_out, w_out)
    #print(conv.shape)

    # #initializing output_res and conv_res
    output_res = np.zeros((h_out*w_out*num,batch_size))
    #print(param["b"].shape)
    for batch in range(batch_size):
        ...
        #print(conv_img)
        res =(np.dot(param['w'].T,conv_img))
         
        #reshaping conv res
        #print(res.shape)
        output_res[:,batch] = res.reshape(-1,h_out*w_out*num)
    #print(output_res.shape)
    output["data"] = output_res
