...
def pooling_layer_forward(input, layer):
    ...
    inpu = input["data"].reshape((h_in,w_in,c,batch_size), order="F")
    pad_inp = [(0, 0), (pad, pad), (pad, pad), (0, 0)]
    input_pad = np.pad(inpu, pad_inp, mode='constant')

    for batch in range(batch_size):
        for h in range(h_out):
            for w in range(w_out):
                for ch in range(c):
                    region = input_pad[(h * stride):(h * stride + k), (w * stride):(w * stride + k), ch,batch]
                    output["data"][h,w,ch,batch] = np.max(region)
